...
import { UserPlus, Mail, Lock, User, Briefcase, Award, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function Register() {
  ...
  // Form datalarını tək bir state-də saxlayırıq
  const [formData, setFormData] = useState({
    fullName: '',
    email: '',
    password: '',
    specialty: 'Bodibildinq / Fitnes', // Default dəyər
    experience: '1-3 il' // Default dəyər
  });
  ...
  const handleRegister = async (e) => {
    ...
    try {
      // 1. Supabase Auth vasitəsilə istifadəçi yaradırıq
      ...
      if (authData?.user) {
        // 2. Auth uğurludursa, məşqçinin digər məlumatlarını 'traineraz' cədvəlinə yazırıq
        const { error: dbError } = await supabase
          .from('traineraz')
          .insert([
            {
              id: authData.user.id, // Auth-dan gələn unikal ID-ni cədvələ bağlayırıq
              full_name: formData.fullName,
              specialty: formData.specialty,
              experience: formData.experience,
              rating: "5.0", // Yeni məşqçiyə başlanğıc olaraq 5 ulduz veririk
              image_url: "" // İstəsə sonradan paneldə yükləyər
            }
          ]);

        if (dbError) throw dbError;

        // Qeydiyyat tamamilə uğurludur! Birbaşa login səhifəsinə yönləndiririk
        alert("Qeydiyyat uğurludur! İndi giriş edə bilərsiniz.");
        navigate('/login');
      }
    } catch (err) {
      console.error("Qeydiyyat xətası:", err.message);
      setErrorMsg(err.message || "Qeydiyyat zamanı gözlənilməz xəta baş verdi.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-zinc-950 text-white flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg bg-zinc-900/40 border border-zinc-900 backdrop-blur-xl rounded-3xl p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative z-10">
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full bg-zinc-950 border border-zinc-800/80 rounded-xl pl-12 pr-4 py-3 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all font-medium appearance-none cursor-pointer text-zinc-300"
              >
          ...
          {/* Təcrübə (Dropdown) */}
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">Təcrübə Müddəti</label>
            <div className="relative">
